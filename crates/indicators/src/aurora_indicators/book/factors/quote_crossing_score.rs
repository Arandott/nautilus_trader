...
const NAME: &str = "quote_crossing_score";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
const TRADE_ONLY: bool = false;

/// Formula/logic: rolling push: if input.bid_prices[0] >= input.ask_prices[0] { 1.0 } else { 0.0 }; value = safe_div(rolling_a.sum(), rolling_a.len() as f64, DEFAULT_EPS)
/// Inputs: ask_price, bid_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct QuoteCrossingScore {
    state: BookFactorState,
}

impl QuoteCrossingScore {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let ask0 = book
            ...
            .map(|price| price.as_f64())
            .unwrap_or(0.0);
        let flag = if bid0 >= ask0 { 1.0 } else { 0.0 };
...
impl Display for QuoteCrossingScore {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for QuoteCrossingScore {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for QuoteCrossingScore {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::QuoteCrossingScore;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_quote_crossing_score_value() {
        let book = sample_book();
        let mut factor = QuoteCrossingScore::new();
        ...
        let ask0 = book
            ...
            .unwrap_or(0.0);
        let flag = if bid0 >= ask0 { 1.0 } else { 0.0 };
        let expected = safe_div(flag, 1.0, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_quote_crossing_score_empty_book() {
        let book = empty_book();
        let mut factor = QuoteCrossingScore::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
