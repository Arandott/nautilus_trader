...
const NAME: &str = "ask_reg_residual_stats";
...
#[derive(Debug)]
pub struct AskRegResidualStats {
    state: BookFactorState,
}

impl AskRegResidualStats {
    #[must_use]
    pub fn new() -> Self {
        Self {
...
impl Display for AskRegResidualStats {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for AskRegResidualStats {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for AskRegResidualStats {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::AskRegResidualStats;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_ask_reg_residual_stats_value() {
        let book = sample_book();
        let mut factor = AskRegResidualStats::new();
        ...
        let x = index_array::<5>();
        let slope = ols_slope(&x, &ask_prices[..5]);
        let intercept = mean(&ask_prices[..5]) - slope * mean(&x);
        let mut residuals = [0.0; 5];
        for i in 0..5 {
            residuals[i] = ask_prices[i] - (intercept + slope * x[i]);
        }
    ...
    #[test]
    fn test_ask_reg_residual_stats_empty_book() {
        let book = empty_book();
        let mut factor = AskRegResidualStats::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
