...
const NAME: &str = "ret_vpin_diff";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
const TRADE_ONLY: bool = false;

/// Formula/logic: rolling push: if delta > 0.0 { vol } else { 0.0 }; if delta < 0.0 { vol } else { 0.0 }; input.ret.or(last_ret); value = vpin - std_ret
/// Inputs: price, ret, volume
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct RetVpinDiff {
...
impl RetVpinDiff {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(price), Some(vol)) = (last_price, volume) {
            ...
            let buy = if delta > 0.0 { vol } else { 0.0 };
            let sell = if delta < 0.0 { vol } else { 0.0 };
            self.state.rolling_a.push(buy);
            self.state.rolling_b.push(sell);
            if let Some(ret) = ret {
                self.state.rolling_c.push(ret);
            }
            let buy_sum = self.state.rolling_a.sum();
            let sell_sum = self.state.rolling_b.sum();
            let vpin = safe_div((buy_sum - sell_sum).abs(), buy_sum + sell_sum, DEFAULT_EPS);
            let std_ret = self.state.rolling_c.std();
            self.state.value = vpin - std_ret;
...
impl Display for RetVpinDiff {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for RetVpinDiff {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for RetVpinDiff {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::RetVpinDiff;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_ret_vpin_diff_value() {
        let book = sample_book();
        let mut factor = RetVpinDiff::new();
        let base_price = book.best_bid_price().unwrap().as_f64();
        factor.state.last_price = Some(base_price - 1.0);
        factor.state.last_trade_price = Some(base_price);
        factor.state.last_trade_volume = Some(10.0);
        factor.state.last_ret = Some(0.01);
        factor.handle_book(&book);
        let buy_sum: f64 = 10.0;
        let sell_sum: f64 = 0.0;
        let vpin = safe_div((buy_sum - sell_sum).abs(), buy_sum + sell_sum, DEFAULT_EPS);
        let std_ret = std(&[0.01]);
        let expected = vpin - std_ret;
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_ret_vpin_diff_empty_book() {
        let book = empty_book();
        let mut factor = RetVpinDiff::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
