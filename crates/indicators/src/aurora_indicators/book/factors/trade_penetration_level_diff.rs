...
const NAME: &str = "trade_penetration_level_diff";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: value = ask_levels - bid_levels
/// Inputs: price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct TradePenetrationLevelDiff {
    state: BookFactorState,
}

impl TradePenetrationLevelDiff {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(price) = last_price {
            ...
            for level in book.asks(Some(BOOK_LEVELS)) {
                let level_price = level.price.value.as_f64();
                if level_price > 0.0 && level_price <= price {
                    ask_levels += 1;
                }
            }
            for level in book.bids(Some(BOOK_LEVELS)) {
                let level_price = level.price.value.as_f64();
                if level_price > 0.0 && level_price >= price {
                    bid_levels += 1;
                }
            }
            self.state.value = ask_levels as f64 - bid_levels as f64;
...
impl Display for TradePenetrationLevelDiff {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for TradePenetrationLevelDiff {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for TradePenetrationLevelDiff {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::TradePenetrationLevelDiff;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_trade_penetration_level_diff_value() {
        let book = sample_book();
        let mut factor = TradePenetrationLevelDiff::new();
        let trade_price = book.best_ask_price().unwrap().as_f64();
        factor.state.last_trade_price = Some(trade_price);
        factor.handle_book(&book);
        let mut ask_levels = 0usize;
        let mut bid_levels = 0usize;
        for level in book.asks(Some(BOOK_LEVELS)) {
            let level_price = level.price.value.as_f64();
            if level_price > 0.0 && level_price <= trade_price {
                ask_levels += 1;
            }
        }
        for level in book.bids(Some(BOOK_LEVELS)) {
        ...
        let expected = ask_levels as f64 - bid_levels as f64;
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_trade_penetration_level_diff_empty_book() {
        let book = empty_book();
        let mut factor = TradePenetrationLevelDiff::new();
