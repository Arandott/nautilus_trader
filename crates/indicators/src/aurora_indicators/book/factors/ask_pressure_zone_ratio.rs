...
const NAME: &str = "ask_pressure_zone_ratio";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: value = safe_div(sum, total, DEFAULT_EPS)
/// Inputs: ask_amt, ask_price, mid_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct AskPressureZoneRatio {
    state: BookFactorState,
}

impl AskPressureZoneRatio {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(mid) = mid_price {
            ...
            self.state.value = safe_div(sum, total, DEFAULT_EPS);
...
impl Display for AskPressureZoneRatio {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for AskPressureZoneRatio {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for AskPressureZoneRatio {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::AskPressureZoneRatio;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_ask_pressure_zone_ratio_value() {
        let book = sample_book();
        let mut factor = AskPressureZoneRatio::new();
        ...
        if let Some(mid) = mid_price {
            ...
            expected = safe_div(sum, total, DEFAULT_EPS);
        }
    ...
    #[test]
    fn test_ask_pressure_zone_ratio_empty_book() {
        let book = empty_book();
        let mut factor = AskPressureZoneRatio::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
