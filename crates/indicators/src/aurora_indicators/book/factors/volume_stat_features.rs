...
const NAME: &str = "volume_stat_features";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: rolling push: vol; value = skew + kurt + std + q
/// Inputs: volume
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct VolumeStatFeatures {
    state: BookFactorState,
}

impl VolumeStatFeatures {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(vol) = volume {
            ...
            let kurt = self.state.rolling_a.kurtosis();
            let std = self.state.rolling_a.std();
            let q = self.state.rolling_a.quantile(0.5);
            self.state.value = skew + kurt + std + q;
...
impl Display for VolumeStatFeatures {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for VolumeStatFeatures {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for VolumeStatFeatures {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::VolumeStatFeatures;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_volume_stat_features_value() {
        let book = sample_book();
        let mut factor = VolumeStatFeatures::new();
        factor.state.last_trade_volume = Some(10.0);
        factor.handle_book(&book);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), 10.0);
    }

    #[test]
    fn test_volume_stat_features_empty_book() {
        let book = empty_book();
        let mut factor = VolumeStatFeatures::new();
        factor.state.last_trade_volume = Some(10.0);
        factor.handle_book(&book);
