...
const NAME: &str = "bid_sweep_ratio";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: value = 1.0 - safe_div(input.bid_amts[0], denom, DEFAULT_EPS)
/// Inputs: bid_amt
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct BidSweepRatio {
...
impl BidSweepRatio {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let mut bid0 = 0.0;
        let mut bid1 = 0.0;
        for (idx, level) in book.bids(Some(2)).enumerate() {
            let amt = level.size();
            match idx {
                0 => bid0 = amt,
                1 => bid1 = amt,
        ...
        let denom = bid0 + bid1;
        self.state.value = 1.0 - safe_div(bid0, denom, DEFAULT_EPS);
...
impl Display for BidSweepRatio {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for BidSweepRatio {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for BidSweepRatio {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::BidSweepRatio;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_bid_sweep_ratio_value() {
        let book = sample_book();
        let mut factor = BidSweepRatio::new();
        ...
        let expected = 1.0 - safe_div(bid0, bid0 + bid1, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_bid_sweep_ratio_empty_book() {
        let book = empty_book();
        let mut factor = BidSweepRatio::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
