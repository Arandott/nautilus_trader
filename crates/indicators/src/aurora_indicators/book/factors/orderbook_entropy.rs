...
impl OrderbookEntropy {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if total > DEFAULT_EPS {
            let mut entropy = 0.0;
            for level in book.bids(Some(5)) {
                let p = safe_div(level.size(), total, DEFAULT_EPS);
                if p > 0.0 {
                    entropy -= p * p.ln();
                }
            }
...
impl Display for OrderbookEntropy {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for OrderbookEntropy {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for OrderbookEntropy {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::OrderbookEntropy;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_orderbook_entropy_value() {
        let book = sample_book();
        let mut factor = OrderbookEntropy::new();
    ...
    #[test]
    fn test_orderbook_entropy_empty_book() {
        let book = empty_book();
        let mut factor = OrderbookEntropy::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
