...
const NAME: &str = "vwap_mid_diff";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: value = vwap - mid
/// Inputs: mid_price, vwap
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct VwapMidDiff {
...
impl VwapMidDiff {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let vwap = match (
            ...
            (Some(bid_price), Some(bid_size), Some(ask_price), Some(ask_size)) => {
                let bid_price = bid_price.as_f64();
                let ask_price = ask_price.as_f64();
                let bid_size = bid_size.as_f64();
                let ask_size = ask_size.as_f64();
                Some(safe_div(
        ...
        if let (Some(vwap), Some(mid)) = (vwap, mid_price) {
            self.state.value = vwap - mid;
...
impl Display for VwapMidDiff {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for VwapMidDiff {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for VwapMidDiff {
    fn value(&self) -> f64 {
        self.state.value
    }
}
