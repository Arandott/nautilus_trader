...
const NAME: &str = "orderbook_convexity_bid";
...
#[derive(Debug)]
pub struct OrderbookConvexityBid {
    state: BookFactorState,
}

impl OrderbookConvexityBid {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let mut prev_amt = None;
        for level in book.bids(Some(5)) {
            let price = level.price.value.as_f64();
            let amt = level.size();
            if let (Some(prev_price), Some(prev_amt)) = (prev_price, prev_amt) {
                let num = prev_price - price;
                let denom = prev_amt + amt;
                sum += safe_div(num, denom, DEFAULT_EPS);
                count += 1.0;
            }
...
impl Display for OrderbookConvexityBid {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for OrderbookConvexityBid {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for OrderbookConvexityBid {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::OrderbookConvexityBid;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_orderbook_convexity_bid_value() {
        let book = sample_book();
        let mut factor = OrderbookConvexityBid::new();
    ...
    #[test]
    fn test_orderbook_convexity_bid_empty_book() {
        let book = empty_book();
        let mut factor = OrderbookConvexityBid::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
