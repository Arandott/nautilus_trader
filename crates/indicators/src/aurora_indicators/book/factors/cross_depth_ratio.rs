...
const NAME: &str = "cross_depth_ratio";
...
#[derive(Debug)]
pub struct CrossDepthRatio {
    state: BookFactorState,
}

impl CrossDepthRatio {
    #[must_use]
    pub fn new() -> Self {
        Self {
...
impl Display for CrossDepthRatio {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for CrossDepthRatio {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for CrossDepthRatio {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::CrossDepthRatio;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_cross_depth_ratio_value() {
        let book = sample_book();
        let mut factor = CrossDepthRatio::new();
    ...
    #[test]
    fn test_cross_depth_ratio_empty_book() {
        let book = empty_book();
        let mut factor = CrossDepthRatio::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
