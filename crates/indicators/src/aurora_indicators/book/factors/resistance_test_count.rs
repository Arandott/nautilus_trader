...
const NAME: &str = "resistance_test_count";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
const TRADE_ONLY: bool = false;

/// Formula/logic: rolling push: if diff <= 1e-6 { 1.0 } else { 0.0 }; value = rolling_a.sum()
/// Inputs: ask_price, price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct ResistanceTestCount {
    state: BookFactorState,
}

impl ResistanceTestCount {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(price) = last_price {
            let ask0 = book
                ...
                .unwrap_or(0.0);
            let diff = (price - ask0).abs();
            let flag = if diff <= 1e-6 { 1.0 } else { 0.0 };
...
impl Display for ResistanceTestCount {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for ResistanceTestCount {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for ResistanceTestCount {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::ResistanceTestCount;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_resistance_test_count_value() {
        let book = sample_book();
        let mut factor = ResistanceTestCount::new();
        let ask0 = book
            .best_ask_price()
            .map(|price| price.as_f64())
            .unwrap_or(0.0);
        factor.state.last_trade_price = Some(ask0);
    ...
    #[test]
    fn test_resistance_test_count_empty_book() {
        let book = empty_book();
        let mut factor = ResistanceTestCount::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
