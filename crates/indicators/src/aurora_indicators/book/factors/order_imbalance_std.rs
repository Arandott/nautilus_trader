...
const NAME: &str = "order_imbalance_std";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: rolling push: safe_div(bid - ask, bid + ask, DEFAULT_EPS); value = rolling_a.std()
/// Inputs: ask_amt, bid_amt
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct OrderImbalanceStd {
    state: BookFactorState,
}

impl OrderImbalanceStd {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let depth = 5;
        let bid: f64 = book.bids(Some(depth)).map(|level| level.size()).sum();
        let ask: f64 = book.asks(Some(depth)).map(|level| level.size()).sum();
        let imbalance = safe_div(bid - ask, bid + ask, DEFAULT_EPS);
...
impl Display for OrderImbalanceStd {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for OrderImbalanceStd {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for OrderImbalanceStd {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::OrderImbalanceStd;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_order_imbalance_std_value() {
        let book = sample_book();
        let mut factor = OrderImbalanceStd::new();
    ...
    #[test]
    fn test_order_imbalance_std_empty_book() {
        let book = empty_book();
        let mut factor = OrderImbalanceStd::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
