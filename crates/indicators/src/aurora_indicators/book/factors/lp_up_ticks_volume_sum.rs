...
const NAME: &str = "lp_up_ticks_volume_sum";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: rolling push: deviation * vol; value = rolling_a.sum()
/// Inputs: ask_price, price, volume
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct LpUpTicksVolumeSum {
    state: BookFactorState,
}

impl LpUpTicksVolumeSum {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(price), Some(vol)) = (last_price, volume) {
            ...
            if delta > 0.0 {
                let deviation = price - ask0;
                self.state.rolling_a.push(deviation * vol);
...
impl Display for LpUpTicksVolumeSum {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for LpUpTicksVolumeSum {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for LpUpTicksVolumeSum {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    #[test]
    fn test_lp_up_ticks_volume_sum_value() {
        let book = sample_book();
        let mut factor = LpUpTicksVolumeSum::new();
        let mut trade = TradeTick::default();
        trade.price = Price::from("100.0");
        trade.size = Quantity::from("5.0");
        factor.handle_trade(&trade);
        factor.handle_book(&book);

        trade.price = Price::from("102.0");
        trade.size = Quantity::from("5.0");
        factor.handle_trade(&trade);
        factor.handle_book(&book);

        let ask0 = book.best_ask_price().map(|price| price.as_f64()).unwrap_or(0.0);
        let expected = (102.0 - ask0) * 5.0;
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_lp_up_ticks_volume_sum_empty_book() {
        let book = empty_book();
        let mut factor = LpUpTicksVolumeSum::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
