...
const NAME: &str = "reverse_tick_ratio";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
const TRADE_ONLY: bool = false;

/// Formula/logic: rolling push: vol; if ret < 0.0 && vol > mean_vol { 1.0 } else { 0.0 }; value = safe_div(rolling_b.sum(), rolling_b.len() as f64, DEFAULT_EPS)
/// Inputs: ret, volume
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct ReverseTickRatio {
    state: BookFactorState,
}

impl ReverseTickRatio {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(ret), Some(vol)) = (ret, volume) {
            ...
            let flag = if ret < 0.0 && vol > mean_vol { 1.0 } else { 0.0 };
            self.state.rolling_b.push(flag);
            self.state.value = safe_div(self.state.rolling_b.sum(), self.state.rolling_b.len() as f64, DEFAULT_EPS);
...
impl Display for ReverseTickRatio {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for ReverseTickRatio {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for ReverseTickRatio {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::ReverseTickRatio;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_reverse_tick_ratio_value() {
        let book = sample_book();
        let mut factor = ReverseTickRatio::new();
    ...
    #[test]
    fn test_reverse_tick_ratio_empty_book() {
        let book = empty_book();
        let mut factor = ReverseTickRatio::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
