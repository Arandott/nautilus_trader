...
const NAME: &str = "return_duration_skew";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
const TRADE_ONLY: bool = false;

/// Formula/logic: rolling push: direction_len as f64; value = rolling_a.skew()
/// Inputs: ret
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct ReturnDurationSkew {
    state: BookFactorState,
}

impl ReturnDurationSkew {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(ret) = ret {
            let sign_now = sign(ret);
            if sign_now == 0.0 {
                self.state.direction_len += 1;
            } else if sign_now == self.state.direction_sign {
                self.state.direction_len += 1;
            } else {
                if self.state.direction_len > 0 {
                    self.state.rolling_a.push(self.state.direction_len as f64);
                }
                self.state.direction_sign = sign_now;
                self.state.direction_len = 1;
            }
...
impl Display for ReturnDurationSkew {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for ReturnDurationSkew {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for ReturnDurationSkew {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::ReturnDurationSkew;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_return_duration_skew_value() {
        let book = sample_book();
        let mut factor = ReturnDurationSkew::new();
    ...
    #[test]
    fn test_return_duration_skew_empty_book() {
        let book = empty_book();
        let mut factor = ReturnDurationSkew::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
