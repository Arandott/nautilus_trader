...
const NAME: &str = "depth_decay_ratio";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = safe_div(input.bid_amts[0], input.bid_amts[4], DEFAULT_EPS)
/// Inputs: bid_amt
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct DepthDecayRatio {
    state: BookFactorState,
}

impl DepthDecayRatio {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        self.state.value = safe_div(bid0, bid4, DEFAULT_EPS);
...
impl Display for DepthDecayRatio {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for DepthDecayRatio {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for DepthDecayRatio {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::DepthDecayRatio;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_depth_decay_ratio_value() {
        let book = sample_book();
        let mut factor = DepthDecayRatio::new();
        ...
        let expected = safe_div(bid0, bid4, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_depth_decay_ratio_empty_book() {
        let book = empty_book();
        let mut factor = DepthDecayRatio::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
