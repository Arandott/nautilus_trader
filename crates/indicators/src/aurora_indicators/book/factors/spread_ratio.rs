...
const NAME: &str = "spread_ratio";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: value = safe_div(bid - ask, mid, DEFAULT_EPS)
/// Inputs: ask_price, bid_price, mid_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct SpreadRatio {
...
impl SpreadRatio {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let (mid_price, spread, bid_price, ask_price) =
        ...
        if let (Some(mid), Some(bid), Some(ask)) = (mid_price, bid_price, ask_price) {
            self.state.value = safe_div(bid - ask, mid, DEFAULT_EPS);
...
impl Display for SpreadRatio {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for SpreadRatio {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for SpreadRatio {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::SpreadRatio;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_spread_ratio_value() {
        let book = sample_book();
        let mut factor = SpreadRatio::new();
        factor.handle_book(&book);
        let bid = book.best_bid_price().unwrap().as_f64();
        let ask = book.best_ask_price().unwrap().as_f64();
        let mid = (bid + ask) * 0.5;
        let expected = safe_div(bid - ask, mid, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_spread_ratio_empty_book() {
        let book = empty_book();
        let mut factor = SpreadRatio::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
