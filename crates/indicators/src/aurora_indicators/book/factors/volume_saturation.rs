...
const NAME: &str = "volume_saturation";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: value = safe_div(vol, depth_sum, DEFAULT_EPS)
/// Inputs: ask_amt, bid_amt, volume
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct VolumeSaturation {
    state: BookFactorState,
}

impl VolumeSaturation {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(vol) = volume {
            ...
            let depth_sum = bid_sum + ask_sum;
            self.state.value = safe_div(vol, depth_sum, DEFAULT_EPS);
...
impl Display for VolumeSaturation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for VolumeSaturation {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for VolumeSaturation {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::VolumeSaturation;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_volume_saturation_value() {
        let book = sample_book();
        let mut factor = VolumeSaturation::new();
        ...
        let expected = safe_div(10.0, bid_sum + ask_sum, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_volume_saturation_empty_book() {
        let book = empty_book();
        let mut factor = VolumeSaturation::new();
        factor.state.last_trade_volume = Some(10.0);
        factor.handle_book(&book);
