...
const NAME: &str = "book_compression_rate";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: value = safe_div(used, 25.0, DEFAULT_EPS)
/// Inputs: ask_amt, bid_amt
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct BookCompressionRate {
    state: BookFactorState,
}

impl BookCompressionRate {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        self.state.value = safe_div(used, BOOK_LEVELS as f64, DEFAULT_EPS);
...
impl Display for BookCompressionRate {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for BookCompressionRate {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for BookCompressionRate {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::BookCompressionRate;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_book_compression_rate_value() {
        let book = sample_book();
        let mut factor = BookCompressionRate::new();
        ...
        let expected = safe_div(used, BOOK_LEVELS as f64, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_book_compression_rate_empty_book() {
        let book = empty_book();
        let mut factor = BookCompressionRate::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
