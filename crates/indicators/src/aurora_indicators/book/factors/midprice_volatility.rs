...
const NAME: &str = "midprice_volatility";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: rolling push: mid; value = rolling_a.std()
/// Inputs: mid_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct MidpriceVolatility {
    state: BookFactorState,
}

impl MidpriceVolatility {
    #[must_use]
    pub fn new() -> Self {
        Self {
...
impl Display for MidpriceVolatility {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for MidpriceVolatility {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for MidpriceVolatility {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::MidpriceVolatility;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_midprice_volatility_value() {
        let book = sample_book();
        let mut factor = MidpriceVolatility::new();
    ...
    #[test]
    fn test_midprice_volatility_empty_book() {
        let book = empty_book();
        let mut factor = MidpriceVolatility::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
