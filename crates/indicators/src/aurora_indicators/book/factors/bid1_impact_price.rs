...
const NAME: &str = "bid1_impact_price";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: value = safe_div(input.bid_amts[0], denom, DEFAULT_EPS)
/// Inputs: bid_amt, bid_price, price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct Bid1ImpactPrice {
    state: BookFactorState,
}

impl Bid1ImpactPrice {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let mut bid0_price = 0.0;
        let mut bid0_amt = 0.0;
        if let Some(level) = book.bids(Some(1)).next() {
            bid0_price = level.price.value.as_f64();
            bid0_amt = level.size();
        }
        if let Some(price) = last_price {
            let denom = price - bid0_price;
            self.state.value = safe_div(bid0_amt, denom, DEFAULT_EPS);
...
impl Display for Bid1ImpactPrice {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for Bid1ImpactPrice {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for Bid1ImpactPrice {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::Bid1ImpactPrice;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_bid1_impact_price_value() {
        let book = sample_book();
        let mut factor = Bid1ImpactPrice::new();
        factor.state.last_trade_price = Some(100.6);
        ...
        let expected = safe_div(bid0_amt, 100.6 - bid0_price, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_bid1_impact_price_empty_book() {
        let book = empty_book();
        let mut factor = Bid1ImpactPrice::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
