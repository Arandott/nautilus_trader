...
const NAME: &str = "orderbook_volume_imbalance_L";
...
#[derive(Debug)]
pub struct OrderbookVolumeImbalanceL {
    state: BookFactorState,
}

impl OrderbookVolumeImbalanceL {
    #[must_use]
    pub fn new() -> Self {
        Self {
...
impl Display for OrderbookVolumeImbalanceL {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for OrderbookVolumeImbalanceL {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for OrderbookVolumeImbalanceL {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::OrderbookVolumeImbalanceL;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_orderbook_volume_imbalance_l_value() {
        let book = sample_book();
        let mut factor = OrderbookVolumeImbalanceL::new();
    ...
    #[test]
    fn test_orderbook_volume_imbalance_l_empty_book() {
        let book = empty_book();
        let mut factor = OrderbookVolumeImbalanceL::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
