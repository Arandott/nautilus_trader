...
const NAME: &str = "volume_density_per_spread";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: value = safe_div(vol, spread, DEFAULT_EPS)
/// Inputs: spread, volume
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct VolumeDensityPerSpread {
    state: BookFactorState,
}

impl VolumeDensityPerSpread {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(vol), Some(spread)) = (volume, spread) {
            self.state.value = safe_div(vol, spread, DEFAULT_EPS);
...
impl Display for VolumeDensityPerSpread {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for VolumeDensityPerSpread {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for VolumeDensityPerSpread {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::VolumeDensityPerSpread;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_volume_density_per_spread_value() {
        let book = sample_book();
        let mut factor = VolumeDensityPerSpread::new();
        factor.state.last_trade_volume = Some(10.0);
        factor.handle_book(&book);
        let (_mid, spread): (Option<f64>, Option<f64>) = mid_and_spread(&book);
        let expected = safe_div(10.0, spread.unwrap(), DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_volume_density_per_spread_empty_book() {
        let book = empty_book();
        let mut factor = VolumeDensityPerSpread::new();
        factor.state.last_trade_volume = Some(10.0);
        factor.handle_book(&book);
