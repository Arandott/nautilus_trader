...
const NAME: &str = "orderbook_amount_imbalance_std_L";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
const TRADE_ONLY: bool = false;

/// Formula/logic: rolling push: input.bid_notional_sum(depth) - input.ask_notional_sum(depth); value = rolling_a.std()
/// Inputs: ask_amt, ask_price, bid_amt, bid_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct OrderbookAmountImbalanceStdL {
    state: BookFactorState,
}

impl OrderbookAmountImbalanceStdL {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        for level in book.bids(Some(depth)) {
            bid_notional += level.price.value.as_f64() * level.size();
        }
        let mut ask_notional = 0.0;
        for level in book.asks(Some(depth)) {
            ask_notional += level.price.value.as_f64() * level.size();
        }
        let imbalance = bid_notional - ask_notional;
        self.state.rolling_a.push(imbalance);
        self.state.value = self.state.rolling_a.std();
...
impl Display for OrderbookAmountImbalanceStdL {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for OrderbookAmountImbalanceStdL {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for OrderbookAmountImbalanceStdL {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::OrderbookAmountImbalanceStdL;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_orderbook_amount_imbalance_std_l_value() {
        let book = sample_book();
        let mut factor = OrderbookAmountImbalanceStdL::new();
        ...
        let imbalance = bid_notional - ask_notional;
        let expected = std(&[imbalance]);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_orderbook_amount_imbalance_std_l_empty_book() {
        let book = empty_book();
        let mut factor = OrderbookAmountImbalanceStdL::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
