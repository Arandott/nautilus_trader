...
const NAME: &str = "amihud_illiquidity";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: rolling push: safe_div(ret.abs(), price * vol, DEFAULT_EPS); value = rolling_a.mean()
...
impl AmihudIlliquidity {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(ret), Some(price), Some(vol)) = (ret, last_price, volume) {
            let illiq = safe_div(ret.abs(), price * vol, DEFAULT_EPS);
            self.state.rolling_a.push(illiq);
...
impl Display for AmihudIlliquidity {
...
impl ValueIndicator for AmihudIlliquidity {
    fn value(&self) -> f64 {
        self.state.value
    }
}
