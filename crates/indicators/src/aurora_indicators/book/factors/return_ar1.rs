...
const NAME: &str = "return_ar1";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
const TRADE_ONLY: bool = false;

/// Formula/logic: rolling push: prev_ret; input.ret.or(last_ret); prev_ret, ret; value = safe_div(corr * std_y, std_x, DEFAULT_EPS)
/// Inputs: ret
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct ReturnAr1 {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(ret) = ret {
            if let Some(prev_ret) = self.state.last_ret {
                self.state.corr_a.push(prev_ret, ret);
                self.state.rolling_a.push(prev_ret);
                self.state.rolling_b.push(ret);
                let corr = self.state.corr_a.corr();
                let std_x = self.state.rolling_a.std();
                let std_y = self.state.rolling_b.std();
                self.state.value = safe_div(corr * std_y, std_x, DEFAULT_EPS);
...
impl Display for ReturnAr1 {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for ReturnAr1 {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for ReturnAr1 {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    #[test]
    fn test_return_ar1_value() {
        let book = sample_book();
        let mut factor = ReturnAr1::new();
        let ret1 = 0.01;
        let ret2 = 0.02;
        factor.state.last_ret = Some(ret1);
        factor.handle_book(&book);
        factor.state.last_ret = Some(ret2);
        factor.handle_book(&book);
        let std_x = std(&[ret1, ret2]);
        let expected = safe_div(std_x, std_x, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_return_ar1_empty_book() {
        let book = empty_book();
        let mut factor = ReturnAr1::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
