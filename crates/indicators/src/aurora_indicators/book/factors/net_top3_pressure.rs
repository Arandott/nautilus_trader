...
const NAME: &str = "net_top3_pressure";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = safe_div(ask_sum - bid_sum, ask_sum + bid_sum, DEFAULT_EPS)
/// Inputs: ask_amt, bid_amt
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct NetTop3Pressure {
    state: BookFactorState,
}

impl NetTop3Pressure {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let depth = 3;
        let bid_sum: f64 = book.bids(Some(depth)).map(|level| level.size()).sum();
        let ask_sum: f64 = book.asks(Some(depth)).map(|level| level.size()).sum();
        self.state.value = safe_div(ask_sum - bid_sum, ask_sum + bid_sum, DEFAULT_EPS);
...
impl Display for NetTop3Pressure {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for NetTop3Pressure {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for NetTop3Pressure {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::NetTop3Pressure;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_net_top3_pressure_value() {
        let book = sample_book();
        let mut factor = NetTop3Pressure::new();
        factor.handle_book(&book);

        let depth = 3;
        let bid_sum: f64 = book.bids(Some(depth)).map(|level| level.size()).sum();
        let ask_sum: f64 = book.asks(Some(depth)).map(|level| level.size()).sum();
        let expected = safe_div(ask_sum - bid_sum, ask_sum + bid_sum, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_net_top3_pressure_empty_book() {
        let book = empty_book();
        let mut factor = NetTop3Pressure::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
