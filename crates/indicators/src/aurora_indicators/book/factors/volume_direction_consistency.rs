...
const NAME: &str = "volume_direction_consistency";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: rolling push: if sign(ret) == sign(vol_delta) { 1.0 } else { 0.0 }; value = safe_div(rolling_a.sum(), rolling_a.len() as f64, DEFAULT_EPS)
/// Inputs: ret, volume
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct VolumeDirectionConsistency {
    state: BookFactorState,
}

impl VolumeDirectionConsistency {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(ret), Some(vol)) = (ret, volume) {
            let vol_delta = self.state.last_volume.map(|prev| vol - prev).unwrap_or(0.0);
            let flag = if sign(ret) == sign(vol_delta) { 1.0 } else { 0.0 };
...
impl Display for VolumeDirectionConsistency {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for VolumeDirectionConsistency {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for VolumeDirectionConsistency {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::VolumeDirectionConsistency;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_volume_direction_consistency_value() {
        let book = sample_book();
        let mut factor = VolumeDirectionConsistency::new();
    ...
    #[test]
    fn test_volume_direction_consistency_empty_book() {
        let book = empty_book();
        let mut factor = VolumeDirectionConsistency::new();
        factor.state.last_trade_volume = Some(10.0);
        factor.state.last_ret = Some(-0.01);
