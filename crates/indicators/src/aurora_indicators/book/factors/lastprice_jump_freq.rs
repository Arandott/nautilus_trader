...
const NAME: &str = "lastprice_jump_freq";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: rolling push: last_price.map(|prev| price - prev).unwrap_or(0.0); value = safe_div(count as f64, rolling_a.len() as f64, DEFAULT_EPS)
/// Inputs: price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct LastpriceJumpFreq {
    state: BookFactorState,
}

impl LastpriceJumpFreq {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(price) = last_price {
            ...
            let count = self.state.rolling_a
                .values()
                .iter()
                .filter(|v| v.abs() > threshold)
                .count();
            self.state.value = safe_div(count as f64, self.state.rolling_a.len() as f64, DEFAULT_EPS);
...
impl Display for LastpriceJumpFreq {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for LastpriceJumpFreq {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for LastpriceJumpFreq {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    #[test]
    fn test_lastprice_jump_freq_value() {
        let book = sample_book();
        let mut factor = LastpriceJumpFreq::new();
    ...
    #[test]
    fn test_lastprice_jump_freq_empty_book() {
        let book = empty_book();
        let mut factor = LastpriceJumpFreq::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
