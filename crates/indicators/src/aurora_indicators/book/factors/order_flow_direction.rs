...
const NAME: &str = "order_flow_direction";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = safe_div(delta, vol, DEFAULT_EPS)
/// Inputs: price, volume
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct OrderFlowDirection {
    state: BookFactorState,
}

impl OrderFlowDirection {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(price), Some(vol)) = (last_price, volume) {
            ...
            self.state.value = safe_div(delta, vol, DEFAULT_EPS);
...
impl Display for OrderFlowDirection {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for OrderFlowDirection {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for OrderFlowDirection {
    fn value(&self) -> f64 {
        self.state.value
    }
}
