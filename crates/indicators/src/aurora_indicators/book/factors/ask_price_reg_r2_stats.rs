...
const NAME: &str = "ask_price_reg_r2_stats";
...
#[derive(Debug)]
pub struct AskPriceRegR2Stats {
    state: BookFactorState,
}

impl AskPriceRegR2Stats {
    #[must_use]
    pub fn new() -> Self {
        Self {
...
impl Display for AskPriceRegR2Stats {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for AskPriceRegR2Stats {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for AskPriceRegR2Stats {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::AskPriceRegR2Stats;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_ask_price_reg_r2_stats_value() {
        let book = sample_book();
        let mut factor = AskPriceRegR2Stats::new();
        ...
        let x = index_array::<5>();
        let expected = ols_r2(&x, &ask_prices[..5]);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_ask_price_reg_r2_stats_empty_book() {
        let book = empty_book();
        let mut factor = AskPriceRegR2Stats::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
