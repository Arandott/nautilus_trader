...
const NAME: &str = "ask_shield_ratio";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: value = safe_div( input.ask_amts[0], input.ask_amts[1] + input.ask_amts[2], DEFAULT_EPS, )
/// Inputs: ask_amt
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct AskShieldRatio {
...
impl AskShieldRatio {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        self.state.value = safe_div(ask0, ask1 + ask2, DEFAULT_EPS);
...
impl Display for AskShieldRatio {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for AskShieldRatio {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for AskShieldRatio {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::AskShieldRatio;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_ask_shield_ratio_value() {
        let book = sample_book();
        let mut factor = AskShieldRatio::new();
        ...
        let expected = safe_div(ask0, ask1 + ask2, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_ask_shield_ratio_empty_book() {
        let book = empty_book();
        let mut factor = AskShieldRatio::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
