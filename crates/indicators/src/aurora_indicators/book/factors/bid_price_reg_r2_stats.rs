...
const NAME: &str = "bid_price_reg_r2_stats";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: rolling push: ols_r2(&x, &y); value = rolling_a.mean()
/// Inputs: bid_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct BidPriceRegR2Stats {
    state: BookFactorState,
}

impl BidPriceRegR2Stats {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let r2 = if ss_tot <= DEFAULT_EPS {
            0.0
        } else {
            1.0 - safe_div(ss_res, ss_tot, DEFAULT_EPS)
        };
        self.state.rolling_a.push(r2);
...
impl Display for BidPriceRegR2Stats {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for BidPriceRegR2Stats {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for BidPriceRegR2Stats {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::BidPriceRegR2Stats;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_bid_price_reg_r2_stats_value() {
        let book = sample_book();
        let mut factor = BidPriceRegR2Stats::new();
        ...
        let x = index_array::<5>();
        let expected = ols_r2(&x, &bid_prices[..5]);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_bid_price_reg_r2_stats_empty_book() {
        let book = empty_book();
        let mut factor = BidPriceRegR2Stats::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
