...
impl CrossSpreadDepth {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let mut sum = 0.0;
        let mut total = 0.0;
        for (idx, level) in book.bids(Some(10)).enumerate() {
            if idx >= BOOK_LEVELS.min(10) {
                break;
            }
...
impl Display for CrossSpreadDepth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for CrossSpreadDepth {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for CrossSpreadDepth {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::CrossSpreadDepth;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_cross_spread_depth_value() {
        let book = sample_book();
        let mut factor = CrossSpreadDepth::new();
        ...
        let expected = safe_div(sum, total, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_cross_spread_depth_empty_book() {
        let book = empty_book();
        let mut factor = CrossSpreadDepth::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
