...
const NAME: &str = "ask_impact_volatility";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: rolling push: input.ask_prices[0]; value = safe_div(std, mean, DEFAULT_EPS)
/// Inputs: ask_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct AskImpactVolatility {
    state: BookFactorState,
}

impl AskImpactVolatility {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let ask0 = book
            ...
            .unwrap_or(0.0);
        self.state.rolling_a.push(ask0);
        let std = self.state.rolling_a.std();
        let mean = self.state.rolling_a.mean();
        self.state.value = safe_div(std, mean, DEFAULT_EPS);
...
impl Display for AskImpactVolatility {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for AskImpactVolatility {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for AskImpactVolatility {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::AskImpactVolatility;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_ask_impact_volatility_value() {
        let book = sample_book();
        let mut factor = AskImpactVolatility::new();
    ...
    #[test]
    fn test_ask_impact_volatility_empty_book() {
        let book = empty_book();
        let mut factor = AskImpactVolatility::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
