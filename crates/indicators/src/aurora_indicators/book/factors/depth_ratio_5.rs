...
const NAME: &str = "depth_ratio_5";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = safe_div(bid_sum, ask_sum, DEFAULT_EPS)
/// Inputs: ask_amt, bid_amt
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct DepthRatio5 {
...
impl DepthRatio5 {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        self.state.value = safe_div(bid_sum, ask_sum, DEFAULT_EPS);
...
impl Display for DepthRatio5 {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for DepthRatio5 {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for DepthRatio5 {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::DepthRatio5;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_depth_ratio_5_value() {
        let book = sample_book();
        let mut factor = DepthRatio5::new();
        ...
        let expected = safe_div(bid_sum, ask_sum, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_depth_ratio_5_empty_book() {
        let book = empty_book();
        let mut factor = DepthRatio5::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
