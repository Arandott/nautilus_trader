...
const NAME: &str = "order_spread_ir";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: rolling push: spread, ret; value = corr_a.corr()
/// Inputs: ret, spread
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct OrderSpreadIr {
...
impl OrderSpreadIr {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(spread), Some(ret)) = (spread, ret) {
            self.state.corr_a.push(spread, ret);
...
impl Display for OrderSpreadIr {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for OrderSpreadIr {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for OrderSpreadIr {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::OrderSpreadIr;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_order_spread_ir_value() {
        let book = sample_book();
        let mut factor = OrderSpreadIr::new();
        factor.state.last_ret = Some(0.01);
        factor.handle_book(&book);

        let (_, spread): (Option<f64>, Option<f64>) = mid_and_spread(&book);
        let mut corr = RollingCorrelation::new(super::WINDOW);
        if let Some(spread) = spread {
            corr.push(spread, 0.01);
        }
        let expected = corr.corr();
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_order_spread_ir_empty_book() {
        let book = empty_book();
        let mut factor = OrderSpreadIr::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
