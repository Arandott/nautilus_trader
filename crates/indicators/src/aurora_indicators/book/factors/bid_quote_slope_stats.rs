...
const NAME: &str = "bid_quote_slope_stats";
...
#[derive(Debug)]
pub struct BidQuoteSlopeStats {
    state: BookFactorState,
}

impl BidQuoteSlopeStats {
    #[must_use]
    pub fn new() -> Self {
        Self {
...
impl Display for BidQuoteSlopeStats {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for BidQuoteSlopeStats {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for BidQuoteSlopeStats {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::BidQuoteSlopeStats;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_bid_quote_slope_stats_value() {
        let book = sample_book();
        let mut factor = BidQuoteSlopeStats::new();
        ...
        let x = index_array::<5>();
        let expected = ols_slope(&x, &bid_prices[..5]);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_bid_quote_slope_stats_empty_book() {
        let book = empty_book();
        let mut factor = BidQuoteSlopeStats::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
