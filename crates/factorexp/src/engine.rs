...
use crate::{
    ...
    operators::{
        RollingOperator, get_pair_rolling_operator, get_rolling_operator,
        pair_rolling::PairRollingOperator,
    },
};
...
impl OperatorNode {
    ...
    pub fn new(
        ...
        let window_size = params
            .get("window")
            .ok_or_else(|| {
                ExpressionError::InvalidParameters(
        ...
        let operator = if operator_name == "TS_Quantile" {
            if let Some(&phi) = params.get("phi") {
                Some(
                    Box::new(crate::operators::rolling::Quantile::new(window_size, phi))
                    ...
                    if let Some(ref mut pair_operator) = self.pair_operator {
                        ...
                        if !input1.is_nan() && !input2.is_nan() {
                            ...
                            if self.staleness_count > STALENESS_THRESHOLD {
                                eprintln!(
                                    "Warning: {} has received {} consecutive invalid values",
...
impl InstantOperatorNode {
    ...
    pub fn new(operator_name: String, children: Vec<Box<dyn ExpressionNode>>) -> Self {
        Self {
            operator_name,
            children,
            current_value: None,
    ...
    fn compute_result(&self, child_values: &[f64]) -> ExpressionResult<f64> {
        match self.operator_name.as_str() {
            ...
            "Sub" => {
                if child_values.len() != 2 {
                    return Err(ExpressionError::InvalidParameters(
                        "Sub requires 2 arguments".to_string(),
                    ));
                }
                Ok(child_values[0] - child_values[1])
            }
            "Mul" => {
                if child_values.len() != 2 {
                    return Err(ExpressionError::InvalidParameters(
                        "Mul requires 2 arguments".to_string(),
                    ));
                }
                Ok(child_values[0] * child_values[1])
            }
            "Div" => {
                if child_values.len() != 2 {
                    return Err(ExpressionError::InvalidParameters(
                        "Div requires 2 arguments".to_string(),
                    ));
                }
            ...
            "Neg" => {
                if child_values.len() != 1 {
                    return Err(ExpressionError::InvalidParameters(
                        "Neg requires 1 argument".to_string(),
                    ));
                }
                Ok(-child_values[0])
            }
            ...
            "Greater" => {
                if child_values.len() != 2 {
                    return Err(ExpressionError::InvalidParameters(
                        "Greater requires 2 arguments".to_string(),
                    ));
                }
                Ok(if child_values[0] > child_values[1] {
                    1.0
                } else {
                    0.0
                })
            ...
            "Less" => {
                if child_values.len() != 2 {
                    return Err(ExpressionError::InvalidParameters(
                        "Less requires 2 arguments".to_string(),
                    ));
                }
                Ok(if child_values[0] < child_values[1] {
                    1.0
            ...
            "GreaterEq" => {
                if child_values.len() != 2 {
                    return Err(ExpressionError::InvalidParameters(
                        "GreaterEq requires 2 arguments".to_string(),
                    ));
                }
                Ok(if child_values[0] >= child_values[1] {
                    1.0
            ...
            "LessEq" => {
                if child_values.len() != 2 {
                    return Err(ExpressionError::InvalidParameters(
                        "LessEq requires 2 arguments".to_string(),
                    ));
                }
                Ok(if child_values[0] <= child_values[1] {
                    1.0
            ...
            "Equal" => {
                if child_values.len() != 2 {
                    return Err(ExpressionError::InvalidParameters(
                        "Equal requires 2 arguments".to_string(),
                    ));
                }
                Ok(if (child_values[0] - child_values[1]).abs() < 1e-10 {
                    1.0
                } else {
                    0.0
                })
            ...
            "NotEqual" => {
                if child_values.len() != 2 {
                    return Err(ExpressionError::InvalidParameters(
                        "NotEqual requires 2 arguments".to_string(),
                    ));
                }
                Ok(if (child_values[0] - child_values[1]).abs() >= 1e-10 {
                    1.0
            ...
            "When" => {
                if child_values.len() != 3 {
                    return Err(ExpressionError::InvalidParameters(
                        "When requires 3 arguments (condition, true_value, false_value)"
                            .to_string(),
                    ));
                }
            ...
            "Clip" => {
                if child_values.len() != 3 {
                    return Err(ExpressionError::InvalidParameters(
                        "Clip requires 3 arguments (value, min, max)".to_string(),
                    ));
                }
                ...
                // Validate bounds
                if min > max {
                    return Err(ExpressionError::InvalidParameters(format!(
                        "Clip min ({}) must be <= max ({})",
                        min, max
                    )));
                }
            ...
            "And" => {
                ...
                Ok(
                    if child_values[0] != 0.0
                        && !child_values[0].is_nan()
                        && child_values[1] != 0.0
                        && !child_values[1].is_nan()
                    {
                        1.0
                    } else {
                        0.0
            ...
            "Or" => {
                ...
                Ok(
                    if (child_values[0] != 0.0 && !child_values[0].is_nan())
...
impl ComputationEngine {
    ...
    fn build_node(&self, node: &ExprNode) -> ExpressionResult<Box<dyn ExpressionNode>> {
        match node {
            ExprNode::Constant(value) => Ok(Box::new(ConstantNode::new(*value))),

            ExprNode::Feature(name) => Ok(Box::new(FeatureNode::new(name.clone()))),
    ...
    pub fn current_value(&self) -> Option<f64> {
        self.root_node
            .as_ref()
            .and_then(|root| root.current_value())
    }
...
mod tests {
    ...
    fn test_clip_operator() {
        ...
        fn compute_clip(engine: &mut ComputationEngine, close: f64) -> Option<f64> {
            let mut buffers = HashMap::new();
            let mut close_buffer = RollingBuffer::new(4);
            close_buffer.push(close);
            buffers.insert("close".to_string(), close_buffer);
            engine.reset();
            engine.update_and_compute(&buffers).unwrap()
        }

        // Test 1: Value below min - should return min (50)
        assert_eq!(compute_clip(&mut engine, 30.0), Some(50.0));

        // Test 2: Value above max - should return max (100)
        assert_eq!(compute_clip(&mut engine, 120.0), Some(100.0));

        // Test 3: Value within range - should return value unchanged
        assert_eq!(compute_clip(&mut engine, 75.0), Some(75.0));

        // Test 4: Edge cases - exactly at bounds
        assert_eq!(compute_clip(&mut engine, 50.0), Some(50.0));
        assert_eq!(compute_clip(&mut engine, 100.0), Some(100.0));
    }
    ...
    fn test_clip_with_dynamic_bounds() {
        ...
        fn compute_dynamic(
            ...
            let mut low_buffer = RollingBuffer::new(4);
            low_buffer.push(low);
            buffers.insert("low".to_string(), low_buffer);

            let mut high_buffer = RollingBuffer::new(4);
            high_buffer.push(high);
        ...
        assert_eq!(compute_dynamic(&mut engine, 105.0, 95.0, 100.0), Some(100.0));
        assert_eq!(compute_dynamic(&mut engine, 90.0, 95.0, 100.0), Some(95.0));
    }
    ...
    fn test_clip_end_to_end_parsing() {
        ...
        fn compute_parsed(engine: &mut ComputationEngine, close: f64) -> Option<f64> {
            let mut buffers = HashMap::new();
            let mut close_buffer = RollingBuffer::new(4);
            close_buffer.push(close);
            buffers.insert("close".to_string(), close_buffer);
        ...
        fn assert_close(actual: Option<f64>, expected: f64) {
            let value = actual.expect("Expected computed value");
            assert!(
        ...
        assert_close(compute_parsed(&mut engine, 95.0), 104.5);
        assert_close(compute_parsed(&mut engine, 105.0), 110.0);
        assert_close(compute_parsed(&mut engine, 80.0), 90.0);
    }
