...
const NAME: &str = "ask_skew_price_weighted";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: value = safe_div(total - top, top, DEFAULT_EPS)
/// Inputs: ask_amt, ask_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct AskSkewPriceWeighted {
    state: BookFactorState,
}

impl AskSkewPriceWeighted {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        for (idx, level) in book.asks(Some(5)).enumerate() {
            ...
            let amt = level.size();
            let notional = price * amt;
            total += notional;
            if idx == 0 {
                top = notional;
            }
        }
        self.state.value = safe_div(total - top, top, DEFAULT_EPS);
...
impl Display for AskSkewPriceWeighted {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for AskSkewPriceWeighted {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for AskSkewPriceWeighted {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::AskSkewPriceWeighted;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_ask_skew_price_weighted_value() {
        let book = sample_book();
        let mut factor = AskSkewPriceWeighted::new();
        ...
        let expected = safe_div(total - top, top, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_ask_skew_price_weighted_empty_book() {
        let book = empty_book();
        let mut factor = AskSkewPriceWeighted::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
