...
const NAME: &str = "volume_spike_score";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: rolling push: vol; value = safe_div(vol - median, std, DEFAULT_EPS)
/// Inputs: volume
...
impl VolumeSpikeScore {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(vol) = volume {
            ...
            let median = self.state.rolling_a.median();
            let std = self.state.rolling_a.std();
            self.state.value = safe_div(vol - median, std, DEFAULT_EPS);
...
impl Display for VolumeSpikeScore {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for VolumeSpikeScore {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for VolumeSpikeScore {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::VolumeSpikeScore;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_volume_spike_score_value() {
        let book = sample_book();
        let mut factor = VolumeSpikeScore::new();
    ...
    #[test]
    fn test_volume_spike_score_empty_book() {
        let book = empty_book();
        let mut factor = VolumeSpikeScore::new();
        factor.state.last_trade_volume = Some(10.0);
        factor.handle_book(&book);
