...
const NAME: &str = "corr_price_volume";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: rolling push: delta, vol; value = corr_a.corr()
/// Inputs: price, volume
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct CorrPriceVolume {
    state: BookFactorState,
}

impl CorrPriceVolume {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(price), Some(vol)) = (last_price, volume) {
            ...
            self.state.corr_a.push(delta, vol);
...
impl Display for CorrPriceVolume {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for CorrPriceVolume {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for CorrPriceVolume {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    #[test]
    fn test_corr_price_volume_value() {
        let book = sample_book();
        let mut factor = CorrPriceVolume::new();
    ...
    #[test]
    fn test_corr_price_volume_empty_book() {
        let book = empty_book();
        let mut factor = CorrPriceVolume::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
