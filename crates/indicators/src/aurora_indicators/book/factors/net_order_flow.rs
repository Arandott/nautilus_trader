...
const NAME: &str = "net_order_flow";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = safe_div(bid_notional - ask_notional, mid, DEFAULT_EPS)
/// Inputs: ask_amt, ask_price, bid_amt, bid_price, mid_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct NetOrderFlow {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(mid) = mid_price {
            ...
            let ask_notional: f64 = book
                ...
                .map(|level| level.price.value.as_f64() * level.size())
                .sum();
            self.state.value = safe_div(bid_notional - ask_notional, mid, DEFAULT_EPS);
...
impl Display for NetOrderFlow {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for NetOrderFlow {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for NetOrderFlow {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::NetOrderFlow;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_net_order_flow_value() {
        let book = sample_book();
        let mut factor = NetOrderFlow::new();
        ...
        let expected = mid_price
            .map(|mid| safe_div(bid_notional - ask_notional, mid, DEFAULT_EPS))
            .unwrap_or(0.0);
    ...
    #[test]
    fn test_net_order_flow_empty_book() {
        let book = empty_book();
        let mut factor = NetOrderFlow::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
