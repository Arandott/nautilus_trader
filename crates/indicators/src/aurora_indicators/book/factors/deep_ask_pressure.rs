...
const NAME: &str = "deep_ask_pressure";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = safe_div(deep_sum, top_sum, DEFAULT_EPS)
/// Inputs: ask_amt
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct DeepAskPressure {
    state: BookFactorState,
}

impl DeepAskPressure {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let mut top_sum = 0.0;
        for (idx, level) in book.asks(Some(15)).enumerate() {
            if idx < 5 {
                top_sum += level.size();
            } else if idx < 15 {
                deep_sum += level.size();
        ...
        self.state.value = safe_div(deep_sum, top_sum, DEFAULT_EPS);
...
impl Display for DeepAskPressure {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for DeepAskPressure {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for DeepAskPressure {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::DeepAskPressure;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_deep_ask_pressure_value() {
        let book = sample_book();
        let mut factor = DeepAskPressure::new();
        ...
        let expected = safe_div(deep_sum, top_sum, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_deep_ask_pressure_empty_book() {
        let book = empty_book();
        let mut factor = DeepAskPressure::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
