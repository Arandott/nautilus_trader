...
const NAME: &str = "book_pressure_gradient";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = safe_div((a2 - a1) - (a1 - a0), mid, DEFAULT_EPS)
/// Inputs: ask_price, mid_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct BookPressureGradient {
    state: BookFactorState,
}

impl BookPressureGradient {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(mid) = mid_price {
            ...
            let mut a1 = 0.0;
            let mut a2 = 0.0;
            for (idx, level) in book.asks(Some(3)).enumerate() {
            ...
            self.state.value = safe_div((a2 - a1) - (a1 - a0), mid, DEFAULT_EPS);
...
impl Display for BookPressureGradient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for BookPressureGradient {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for BookPressureGradient {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::BookPressureGradient;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_book_pressure_gradient_value() {
        let book = sample_book();
        let mut factor = BookPressureGradient::new();
        ...
        let expected = mid_price
            .map(|mid| safe_div((a2 - a1) - (a1 - a0), mid, DEFAULT_EPS))
            .unwrap_or(0.0);
    ...
    #[test]
    fn test_book_pressure_gradient_empty_book() {
        let book = empty_book();
        let mut factor = BookPressureGradient::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
