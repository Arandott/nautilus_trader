...
const NAME: &str = "deep_order_count";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = count as f64
/// Inputs: bid_amt
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct DeepOrderCount {
...
impl DeepOrderCount {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let mean_bid = safe_div(sum, 5.0, DEFAULT_EPS);
        let var = safe_div(sum_sq, 5.0, DEFAULT_EPS) - mean_bid * mean_bid;
        let std_bid = safe_sqrt(var);
        let threshold = mean_bid + std_bid;
        let mut count = 0usize;
        for (idx, level) in book.bids(Some(5)).enumerate() {
            if idx < 5 && level.size() > threshold {
                count += 1;
            }
        }
        self.state.value = count as f64;
...
impl Display for DeepOrderCount {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for DeepOrderCount {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for DeepOrderCount {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::DeepOrderCount;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_deep_order_count_value() {
        let book = sample_book();
        let mut factor = DeepOrderCount::new();
    ...
    #[test]
    fn test_deep_order_count_empty_book() {
        let book = empty_book();
        let mut factor = DeepOrderCount::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
