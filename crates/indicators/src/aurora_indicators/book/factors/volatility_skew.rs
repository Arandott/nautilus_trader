...
const NAME: &str = "volatility_skew";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: rolling push: ret; value = rolling_a.skew()
/// Inputs: ret
...
impl Display for VolatilitySkew {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for VolatilitySkew {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for VolatilitySkew {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::VolatilitySkew;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_volatility_skew_value() {
        let book = sample_book();
        let mut factor = VolatilitySkew::new();
    ...
    #[test]
    fn test_volatility_skew_empty_book() {
        let book = empty_book();
        let mut factor = VolatilitySkew::new();
        factor.state.last_ret = Some(-0.02);
        factor.handle_book(&book);
