...
const NAME: &str = "orderbook_slope_bid";
...
#[derive(Debug)]
pub struct OrderbookSlopeBid {
    state: BookFactorState,
}

impl OrderbookSlopeBid {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        self.state.value = safe_div(bid0 - bid4, bid_sum, DEFAULT_EPS);
...
impl Display for OrderbookSlopeBid {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for OrderbookSlopeBid {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for OrderbookSlopeBid {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::OrderbookSlopeBid;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_orderbook_slope_bid_value() {
        let book = sample_book();
        let mut factor = OrderbookSlopeBid::new();
        ...
        let expected = safe_div(bid0 - bid4, bid_sum, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_orderbook_slope_bid_empty_book() {
        let book = empty_book();
        let mut factor = OrderbookSlopeBid::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
