...
const NAME: &str = "trend_acceleration";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: value = accel
/// Inputs: mid_price
...
impl TrendAcceleration {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(mid) = mid_price {
            let delta = mid - self.state.last_mid.unwrap_or(mid);
            let accel = delta - self.state.last_mid_delta.unwrap_or(0.0);
            self.state.value = accel;
            self.state.initialized = true;
            self.state.last_mid_delta = Some(delta);
        }
...
impl Display for TrendAcceleration {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for TrendAcceleration {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for TrendAcceleration {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::TrendAcceleration;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_trend_acceleration_value() {
        let book = sample_book();
        let mut factor = TrendAcceleration::new();
    ...
    #[test]
    fn test_trend_acceleration_empty_book() {
        let book = empty_book();
        let mut factor = TrendAcceleration::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
