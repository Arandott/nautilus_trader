...
const NAME: &str = "vpin_order_flow";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: rolling push: if delta > 0.0 { vol } else { 0.0 }; if delta < 0.0 { vol } else { 0.0 }; value = safe_div((buy_sum - sell_sum).abs(), buy_sum + sell_sum, DEFAULT_EPS)
/// Inputs: price, volume
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct VpinOrderFlow {
...
impl VpinOrderFlow {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(price), Some(vol)) = (last_price, volume) {
            ...
            self.state.value = safe_div((buy_sum - sell_sum).abs(), buy_sum + sell_sum, DEFAULT_EPS);
...
impl Display for VpinOrderFlow {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for VpinOrderFlow {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for VpinOrderFlow {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::VpinOrderFlow;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_vpin_order_flow_value() {
        let book = sample_book();
        let mut factor = VpinOrderFlow::new();
        factor.state.last_price = Some(99.0);
    ...
    #[test]
    fn test_vpin_order_flow_empty_book() {
        let book = empty_book();
        let mut factor = VpinOrderFlow::new();
        factor.state.last_price = Some(99.0);
        factor.state.last_trade_price = Some(100.0);
