...
const NAME: &str = "volume_ret_corr_lead";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: rolling push: prev_vol, ret; value = corr_a.corr()
/// Inputs: ret, volume
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct VolumeRetCorrLead {
    state: BookFactorState,
}

impl VolumeRetCorrLead {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(ret), Some(_vol)) = (ret, volume) {
            if let Some(prev_vol) = self.state.last_volume {
                self.state.corr_a.push(prev_vol, ret);
...
impl Display for VolumeRetCorrLead {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for VolumeRetCorrLead {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for VolumeRetCorrLead {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::VolumeRetCorrLead;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_volume_ret_corr_lead_value() {
        let book = sample_book();
        let mut factor = VolumeRetCorrLead::new();
        factor.state.last_volume = Some(5.0);
    ...
    #[test]
    fn test_volume_ret_corr_lead_empty_book() {
        let book = empty_book();
        let mut factor = VolumeRetCorrLead::new();
        factor.state.last_volume = Some(5.0);
        factor.state.last_trade_volume = Some(10.0);
