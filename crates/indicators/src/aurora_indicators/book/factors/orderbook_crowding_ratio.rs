...
const NAME: &str = "orderbook_crowding_ratio";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
const TRADE_ONLY: bool = false;

/// Formula/logic: value = safe_div(count as f64, 25.0, DEFAULT_EPS)
/// Inputs: bid_amt
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct OrderbookCrowdingRatio {
    state: BookFactorState,
}

impl OrderbookCrowdingRatio {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let mean_bid = safe_div(sum, BOOK_LEVELS as f64, DEFAULT_EPS);
        let mut count = 0;
        for level in book.bids(Some(BOOK_LEVELS)) {
            if level.size() > mean_bid {
                count += 1;
            }
        }
        self.state.value = safe_div(count as f64, 25.0, DEFAULT_EPS);
...
impl Display for OrderbookCrowdingRatio {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for OrderbookCrowdingRatio {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for OrderbookCrowdingRatio {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::OrderbookCrowdingRatio;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_orderbook_crowding_ratio_value() {
        let book = sample_book();
        let mut factor = OrderbookCrowdingRatio::new();
        ...
        let expected = safe_div(count as f64, 25.0, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_orderbook_crowding_ratio_empty_book() {
        let book = empty_book();
        let mut factor = OrderbookCrowdingRatio::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
