...
const NAME: &str = "orderbook_breathing_rate";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
const TRADE_ONLY: bool = false;

/// Formula/logic: rolling push: delta; value = rolling_a.mean()
/// Inputs: bid_amt
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct OrderbookBreathingRate {
    state: BookFactorState,
}

impl OrderbookBreathingRate {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let bid0 = book
            ...
            .unwrap_or(0.0);
        let delta = self.state.last_bid_amt0
            .map(|prev| (bid0 - prev).abs())
            .unwrap_or(0.0);
        self.state.rolling_a.push(delta);
        self.state.value = self.state.rolling_a.mean();
        self.state.initialized = self.state.rolling_a.is_full();
        self.state.last_bid_amt0 = Some(bid0);
...
impl Display for OrderbookBreathingRate {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for OrderbookBreathingRate {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for OrderbookBreathingRate {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::OrderbookBreathingRate;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_orderbook_breathing_rate_value() {
        let book = sample_book();
        let mut factor = OrderbookBreathingRate::new();
        factor.state.last_bid_amt0 = Some(0.0);
        ...
        assert_relative_eq!(factor.value(), bid0);
    }

    #[test]
    fn test_orderbook_breathing_rate_empty_book() {
        let book = empty_book();
        let mut factor = OrderbookBreathingRate::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
