...
const NAME: &str = "depth_spread_5ask_bid";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = safe_div(avg - ask0, ask0, DEFAULT_EPS)
/// Inputs: ask_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct DepthSpread5askBid {
    state: BookFactorState,
}

impl DepthSpread5askBid {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let avg = safe_div(sum, 4.0, DEFAULT_EPS);
        self.state.value = safe_div(avg - ask0, ask0, DEFAULT_EPS);
...
impl Display for DepthSpread5askBid {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for DepthSpread5askBid {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for DepthSpread5askBid {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::DepthSpread5askBid;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_depth_spread_5ask_bid_value() {
        let book = sample_book();
        let mut factor = DepthSpread5askBid::new();
        ...
        let avg = safe_div(sum, 4.0, DEFAULT_EPS);
        let expected = safe_div(avg - ask0, ask0, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_depth_spread_5ask_bid_empty_book() {
        let book = empty_book();
        let mut factor = DepthSpread5askBid::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
