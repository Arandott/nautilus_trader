...
const NAME: &str = "quote_reversion_speed";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
const TRADE_ONLY: bool = false;

/// Formula/logic: rolling push: last_bid0.map(|prev| (bid0 - prev).abs()).unwrap_or(0.0); value = rolling_a.mean()
/// Inputs: bid_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct QuoteReversionSpeed {
    state: BookFactorState,
}

impl QuoteReversionSpeed {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let bid0 = book
            ...
            .unwrap_or(0.0);
        let delta = self.state.last_bid0.map(|prev| (bid0 - prev).abs()).unwrap_or(0.0);
...
impl Display for QuoteReversionSpeed {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for QuoteReversionSpeed {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for QuoteReversionSpeed {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::QuoteReversionSpeed;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_quote_reversion_speed_value() {
        let book = sample_book();
        let mut factor = QuoteReversionSpeed::new();
        factor.state.last_bid0 = Some(0.0);
    ...
    #[test]
    fn test_quote_reversion_speed_empty_book() {
        let book = empty_book();
        let mut factor = QuoteReversionSpeed::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
