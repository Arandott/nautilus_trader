...
const NAME: &str = "liquidity_gradient_ratio";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = safe_div(bid, ask, DEFAULT_EPS)
/// Inputs: ask_amt, bid_amt
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct LiquidityGradientRatio {
    state: BookFactorState,
}

impl LiquidityGradientRatio {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let bid = bid0 - bid4;
        let ask = ask0 - ask4;
        self.state.value = safe_div(bid, ask, DEFAULT_EPS);
...
impl Display for LiquidityGradientRatio {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for LiquidityGradientRatio {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for LiquidityGradientRatio {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::LiquidityGradientRatio;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_liquidity_gradient_ratio_value() {
        let book = sample_book();
        let mut factor = LiquidityGradientRatio::new();
        ...
        let bid = bid0 - bid4;
        let ask = ask0 - ask4;
        let expected = safe_div(bid, ask, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_liquidity_gradient_ratio_empty_book() {
        let book = empty_book();
        let mut factor = LiquidityGradientRatio::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
