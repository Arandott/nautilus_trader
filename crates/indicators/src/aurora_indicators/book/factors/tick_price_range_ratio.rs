...
const NAME: &str = "tick_price_range_ratio";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: rolling push: price; value = safe_div(max - min, mid, DEFAULT_EPS)
/// Inputs: mid_price, price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct TickPriceRangeRatio {
    state: BookFactorState,
}

impl TickPriceRangeRatio {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(price), Some(mid)) = (last_price, mid_price) {
            self.state.rolling_a.push(price);
            let max = self.state.rolling_a.max();
            let min = self.state.rolling_a.min();
            self.state.value = safe_div(max - min, mid, DEFAULT_EPS);
...
impl Display for TickPriceRangeRatio {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for TickPriceRangeRatio {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for TickPriceRangeRatio {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::TickPriceRangeRatio;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_tick_price_range_ratio_value() {
        let book = sample_book();
        let mut factor = TickPriceRangeRatio::new();
        factor.state.last_trade_price = Some(100.5);
    ...
    #[test]
    fn test_tick_price_range_ratio_empty_book() {
        let book = empty_book();
        let mut factor = TickPriceRangeRatio::new();
        factor.state.last_trade_price = Some(100.5);
        factor.handle_book(&book);
