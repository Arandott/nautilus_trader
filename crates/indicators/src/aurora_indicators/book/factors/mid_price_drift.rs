...
const NAME: &str = "mid_price_drift";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = mid - last_mid.unwrap_or(mid)
/// Inputs: mid_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct MidPriceDrift {
...
impl MidPriceDrift {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(mid) = mid_price {
            self.state.value = mid - self.state.last_mid.unwrap_or(mid);
...
impl Display for MidPriceDrift {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for MidPriceDrift {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for MidPriceDrift {
    fn value(&self) -> f64 {
        self.state.value
    }
}
