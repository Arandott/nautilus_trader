...
const NAME: &str = "dominant_side_tvr";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: rolling push: if delta > 0.0 { vol } else { 0.0 }; vol; value = safe_div(rolling_a.sum(), rolling_b.sum(), DEFAULT_EPS)
/// Inputs: price, volume
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct DominantSideTvr {
    state: BookFactorState,
}

impl DominantSideTvr {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(price), Some(vol)) = (last_price, volume) {
            ...
            self.state.rolling_b.push(vol);
            self.state.value = safe_div(self.state.rolling_a.sum(), self.state.rolling_b.sum(), DEFAULT_EPS);
...
impl Display for DominantSideTvr {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for DominantSideTvr {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for DominantSideTvr {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    #[test]
    fn test_dominant_side_tvr_value() {
        let book = sample_book();
        let mut factor = DominantSideTvr::new();
        ...
        factor.handle_book(&book);
        let expected = safe_div(12.0, 22.0, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_dominant_side_tvr_empty_book() {
        let book = empty_book();
        let mut factor = DominantSideTvr::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
