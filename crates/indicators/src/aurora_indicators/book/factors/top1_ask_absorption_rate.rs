...
const NAME: &str = "top1_ask_absorption_rate";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: rolling push: if price >= best ask price { vol } else { 0.0 }; value = safe_div(rolling_a.sum(), best ask size, DEFAULT_EPS)
/// Inputs: ask_amt, ask_price, price, volume
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct Top1AskAbsorptionRate {
    state: BookFactorState,
}

impl Top1AskAbsorptionRate {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(price), Some(vol)) = (last_price, volume) {
            let flag = if price >= ask_price { vol } else { 0.0 };
            self.state.rolling_a.push(flag);
            self.state.value = safe_div(self.state.rolling_a.sum(), ask_amt, DEFAULT_EPS);
...
impl Display for Top1AskAbsorptionRate {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for Top1AskAbsorptionRate {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for Top1AskAbsorptionRate {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::Top1AskAbsorptionRate;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_top1_ask_absorption_rate_value() {
        let book = sample_book();
        let mut factor = Top1AskAbsorptionRate::new();
        let ask_price = book.best_ask_price().unwrap().as_f64();
        let ask_amt = book.best_ask_size().unwrap().as_f64();
        ...
        let expected = safe_div(5.0, ask_amt, DEFAULT_EPS);
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_top1_ask_absorption_rate_empty_book() {
        let book = empty_book();
        let mut factor = Top1AskAbsorptionRate::new();
