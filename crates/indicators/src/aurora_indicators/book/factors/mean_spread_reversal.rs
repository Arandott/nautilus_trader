...
const NAME: &str = "mean_spread_reversal";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = if ret < 0.0 { spread - prev } else { 0.0 }
/// Inputs: ret, spread
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct MeanSpreadReversal {
    state: BookFactorState,
}

impl MeanSpreadReversal {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(spread), Some(ret)) = (spread, ret) {
            let prev = self.state.last_spread.unwrap_or(spread);
            self.state.value = if ret < 0.0 { spread - prev } else { 0.0 };
...
impl Display for MeanSpreadReversal {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for MeanSpreadReversal {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for MeanSpreadReversal {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::MeanSpreadReversal;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_mean_spread_reversal_value() {
        let book = sample_book();
        let mut factor = MeanSpreadReversal::new();
        factor.state.last_spread = Some(0.8);
        factor.state.last_ret = Some(-0.01);
        factor.handle_book(&book);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), 0.2);
    }

    #[test]
    fn test_mean_spread_reversal_empty_book() {
        let book = empty_book();
        let mut factor = MeanSpreadReversal::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
