...
const NAME: &str = "order_age_indicator";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = age
/// Inputs: order_age
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct OrderAgeIndicator {
    state: BookFactorState,
}

impl OrderAgeIndicator {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook, order_age: Option<f64>) {
...
impl Display for OrderAgeIndicator {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for OrderAgeIndicator {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for OrderAgeIndicator {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::OrderAgeIndicator;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_order_age_indicator_value() {
        let book = sample_book();
        let mut factor = OrderAgeIndicator::new();
        factor.update(&book, Some(12.0));
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), 12.0);
    }

    #[test]
    fn test_order_age_indicator_empty_book() {
        let book = empty_book();
        let mut factor = OrderAgeIndicator::new();
