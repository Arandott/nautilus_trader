...
const NAME: &str = "lvl5_avg_price_ratio_sum_skew";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: rolling push: base; value = rolling_a.skew()
/// Inputs: ask_price, bid_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct Lvl5AvgPriceRatioSumSkew {
    state: BookFactorState,
}

impl Lvl5AvgPriceRatioSumSkew {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        self.state.value = self.state.rolling_a.skew();
...
impl Display for Lvl5AvgPriceRatioSumSkew {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for Lvl5AvgPriceRatioSumSkew {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for Lvl5AvgPriceRatioSumSkew {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::Lvl5AvgPriceRatioSumSkew;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_lvl5_avg_price_ratio_sum_skew_value() {
        let book = sample_book();
        let mut factor = Lvl5AvgPriceRatioSumSkew::new();
    ...
    #[test]
    fn test_lvl5_avg_price_ratio_sum_skew_empty_book() {
        let book = empty_book();
        let mut factor = Lvl5AvgPriceRatioSumSkew::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
