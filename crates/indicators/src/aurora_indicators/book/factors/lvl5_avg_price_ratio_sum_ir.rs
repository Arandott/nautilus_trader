...
const NAME: &str = "lvl5_avg_price_ratio_sum_ir";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: rolling push: base; value = safe_div(mean, std, DEFAULT_EPS)
/// Inputs: ask_price, bid_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct Lvl5AvgPriceRatioSumIr {
    state: BookFactorState,
}

impl Lvl5AvgPriceRatioSumIr {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let base = safe_div(bid_avg, bid0, DEFAULT_EPS) + safe_div(ask_avg, ask0, DEFAULT_EPS);
        self.state.rolling_a.push(base);
        let mean = self.state.rolling_a.mean();
        let std = self.state.rolling_a.std();
        self.state.value = safe_div(mean, std, DEFAULT_EPS);
...
impl Display for Lvl5AvgPriceRatioSumIr {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for Lvl5AvgPriceRatioSumIr {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for Lvl5AvgPriceRatioSumIr {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::Lvl5AvgPriceRatioSumIr;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_lvl5_avg_price_ratio_sum_ir_value() {
        let book = sample_book();
        let mut factor = Lvl5AvgPriceRatioSumIr::new();
        ...
        let ask0 = book.best_ask_price().map(|price| price.as_f64()).unwrap_or(0.0);
        let base = safe_div(bid_avg, bid0, DEFAULT_EPS) + safe_div(ask_avg, ask0, DEFAULT_EPS);
        let mut rolling = RollingWindow::new(super::WINDOW);
        rolling.push(base);
        let expected = safe_div(rolling.mean(), rolling.std(), DEFAULT_EPS);
    ...
    #[test]
    fn test_lvl5_avg_price_ratio_sum_ir_empty_book() {
        let book = empty_book();
        let mut factor = Lvl5AvgPriceRatioSumIr::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
