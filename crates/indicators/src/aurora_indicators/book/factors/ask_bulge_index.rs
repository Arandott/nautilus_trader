...
const NAME: &str = "ask_bulge_index";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: value = safe_div(bulge, top, DEFAULT_EPS)
/// Inputs: ask_amt
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct AskBulgeIndex {
...
impl AskBulgeIndex {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        self.state.value = safe_div(bulge, top, DEFAULT_EPS);
...
impl Display for AskBulgeIndex {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for AskBulgeIndex {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for AskBulgeIndex {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::AskBulgeIndex;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_ask_bulge_index_value() {
        let book = sample_book();
        let mut factor = AskBulgeIndex::new();
        ...
        let expected = safe_div(bulge, top, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_ask_bulge_index_empty_book() {
        let book = empty_book();
        let mut factor = AskBulgeIndex::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
