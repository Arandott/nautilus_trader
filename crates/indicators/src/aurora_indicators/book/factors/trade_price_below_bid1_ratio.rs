...
const NAME: &str = "trade_price_below_bid1_ratio";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: rolling push: if price < best bid price { 1.0 } else { 0.0 }; value = safe_div(rolling_a.sum(), rolling_a.len() as f64, DEFAULT_EPS)
/// Inputs: bid_price, price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct TradePriceBelowBid1Ratio {
    state: BookFactorState,
}

impl TradePriceBelowBid1Ratio {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(price) = last_price {
            let flag = if price < bid_price { 1.0 } else { 0.0 };
...
impl Display for TradePriceBelowBid1Ratio {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for TradePriceBelowBid1Ratio {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for TradePriceBelowBid1Ratio {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::TradePriceBelowBid1Ratio;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_trade_price_below_bid1_ratio_value() {
        let book = sample_book();
        let mut factor = TradePriceBelowBid1Ratio::new();
        let bid_price = book.best_bid_price().unwrap().as_f64();
        factor.state.last_trade_price = Some(bid_price - 0.01);
    ...
    #[test]
    fn test_trade_price_below_bid1_ratio_empty_book() {
        let book = empty_book();
        let mut factor = TradePriceBelowBid1Ratio::new();
