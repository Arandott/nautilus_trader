...
const NAME: &str = "pullback_ratio";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
const TRADE_ONLY: bool = false;

/// Formula/logic: rolling push: mid_price.unwrap_or(price); value = safe_div(max_mid - price, max_mid - min_mid, DEFAULT_EPS)
/// Inputs: mid_price, price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct PullbackRatio {
...
impl PullbackRatio {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        self.state.rolling_a.push(mid_price.unwrap_or(price));
        let max_mid = self.state.rolling_a.max();
        let min_mid = self.state.rolling_a.min();
        self.state.value = safe_div(max_mid - price, max_mid - min_mid, DEFAULT_EPS);
...
impl Display for PullbackRatio {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for PullbackRatio {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for PullbackRatio {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::PullbackRatio;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_pullback_ratio_value() {
        let book = sample_book();
        let mut factor = PullbackRatio::new();
        factor.handle_book(&book);
        let (mid_price, _): (Option<f64>, Option<f64>) = mid_and_spread(&book);
        let price = mid_price.unwrap_or(0.0);
        let max_mid = price;
        let min_mid = price;
        let expected = safe_div(max_mid - price, max_mid - min_mid, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_pullback_ratio_empty_book() {
        let book = empty_book();
        let mut factor = PullbackRatio::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
