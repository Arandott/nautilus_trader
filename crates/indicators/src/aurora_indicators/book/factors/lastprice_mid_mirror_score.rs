...
const NAME: &str = "lastprice_mid_mirror_score";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = safe_div((price - mid).abs(), spread, DEFAULT_EPS)
/// Inputs: mid_price, price, spread
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct LastpriceMidMirrorScore {
    state: BookFactorState,
}

impl LastpriceMidMirrorScore {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let (Some(price), Some(mid)) = (last_price, mid_price) {
            let spread = spread.unwrap_or(0.0).max(DEFAULT_EPS);
            self.state.value = safe_div((price - mid).abs(), spread, DEFAULT_EPS);
...
impl Display for LastpriceMidMirrorScore {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for LastpriceMidMirrorScore {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for LastpriceMidMirrorScore {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::LastpriceMidMirrorScore;
    ...
    #[test]
    fn test_lastprice_mid_mirror_score_value() {
        let book = sample_book();
        let mut factor = LastpriceMidMirrorScore::new();
        ...
        let (mid_price, spread): (Option<f64>, Option<f64>) = mid_and_spread(&book);
        let mid = mid_price.unwrap_or(0.0);
        let spread = spread.unwrap_or(0.0).max(DEFAULT_EPS);
        let expected = safe_div((trade.price.as_f64() - mid).abs(), spread, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_lastprice_mid_mirror_score_empty_book() {
        let book = empty_book();
        let mut factor = LastpriceMidMirrorScore::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
