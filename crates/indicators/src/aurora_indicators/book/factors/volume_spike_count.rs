...
const NAME: &str = "volume_spike_count";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: rolling push: vol; value = count as f64
/// Inputs: volume
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct VolumeSpikeCount {
    state: BookFactorState,
}

impl VolumeSpikeCount {
    #[must_use]
    pub fn new() -> Self {
        Self {
...
impl Display for VolumeSpikeCount {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for VolumeSpikeCount {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for VolumeSpikeCount {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::VolumeSpikeCount;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_volume_spike_count_value() {
        let book = sample_book();
        let mut factor = VolumeSpikeCount::new();
    ...
    #[test]
    fn test_volume_spike_count_empty_book() {
        let book = empty_book();
        let mut factor = VolumeSpikeCount::new();
        factor.state.last_trade_volume = Some(10.0);
        factor.handle_book(&book);
