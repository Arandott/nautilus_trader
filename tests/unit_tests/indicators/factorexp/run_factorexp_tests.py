...
# Import test modules
try:
    from test_factorexp_advanced import TestFactorExpAdvanced
    from test_factorexp_advanced import run_advanced_test_suite
    from test_factorexp_comprehensive import TestFactorExpComprehensive
    ...
    def run_expression_samples(self):
        ...
        for expression, description in sample_expressions:
            try:
                ...
                self.log(f"{description} - Error: {e!s}", "FAIL")
                self.results["failed"] += 1
                self.results["errors"].append({
