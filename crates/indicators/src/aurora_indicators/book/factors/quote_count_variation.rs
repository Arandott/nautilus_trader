...
const NAME: &str = "quote_count_variation";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
const TRADE_ONLY: bool = false;

/// Formula/logic: rolling push: bid_change + ask_change; value = rolling_a.sum()
/// Inputs: ask_price, bid_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct QuoteCountVariation {
    state: BookFactorState,
}

impl QuoteCountVariation {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let ask0 = book
            ...
            .map(|price| price.as_f64())
            .unwrap_or(0.0);
        let bid_change = if Some(bid0) != self.state.last_bid0 { 1.0 } else { 0.0 };
        let ask_change = if Some(ask0) != self.state.last_ask0 { 1.0 } else { 0.0 };
        self.state.rolling_a.push(bid_change + ask_change);
...
impl Display for QuoteCountVariation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for QuoteCountVariation {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for QuoteCountVariation {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::QuoteCountVariation;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_quote_count_variation_value() {
        let book = sample_book();
        let mut factor = QuoteCountVariation::new();
        let bid0 = book
            .best_bid_price()
            .map(|price| price.as_f64())
            .unwrap_or(0.0);
        let ask0 = book
            ...
            .map(|price| price.as_f64())
            .unwrap_or(0.0);
        let bid_change = if Some(bid0) != factor.state.last_bid0 { 1.0 } else { 0.0 };
        let ask_change = if Some(ask0) != factor.state.last_ask0 { 1.0 } else { 0.0 };
        let expected = bid_change + ask_change;
    ...
    #[test]
    fn test_quote_count_variation_empty_book() {
        let book = empty_book();
        let mut factor = QuoteCountVariation::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
