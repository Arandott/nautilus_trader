...
const NAME: &str = "center_of_liquidity_bid";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = safe_div(num, denom, DEFAULT_EPS)
/// Inputs: bid_amt, bid_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct CenterOfLiquidityBid {
    state: BookFactorState,
}

impl CenterOfLiquidityBid {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        self.state.value = safe_div(num, denom, DEFAULT_EPS);
...
impl Display for CenterOfLiquidityBid {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for CenterOfLiquidityBid {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for CenterOfLiquidityBid {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::CenterOfLiquidityBid;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_center_of_liquidity_bid_value() {
        let book = sample_book();
        let mut factor = CenterOfLiquidityBid::new();
        ...
        let expected = safe_div(num, denom, DEFAULT_EPS);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_center_of_liquidity_bid_empty_book() {
        let book = empty_book();
        let mut factor = CenterOfLiquidityBid::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
