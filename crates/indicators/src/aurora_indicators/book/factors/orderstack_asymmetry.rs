...
impl OrderstackAsymmetry {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        let mut bid = 0.0;
        for level in book.bids(Some(10)) {
            bid += level.size();
        }
        let mut ask = 0.0;
        for level in book.asks(Some(10)) {
            ask += level.size();
        }
        self.state.value = bid - ask;
...
impl Display for OrderstackAsymmetry {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for OrderstackAsymmetry {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for OrderstackAsymmetry {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::OrderstackAsymmetry;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_orderstack_asymmetry_value() {
        let book = sample_book();
        let mut factor = OrderstackAsymmetry::new();
