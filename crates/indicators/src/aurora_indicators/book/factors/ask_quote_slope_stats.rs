...
const NAME: &str = "ask_quote_slope_stats";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: rolling push: ols_slope(&x, &y); value = rolling_a.mean()
/// Inputs: ask_price
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct AskQuoteSlopeStats {
    state: BookFactorState,
}

impl AskQuoteSlopeStats {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        self.state.rolling_a.push(slope);
...
impl Display for AskQuoteSlopeStats {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for AskQuoteSlopeStats {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for AskQuoteSlopeStats {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::AskQuoteSlopeStats;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_ask_quote_slope_stats_value() {
        let book = sample_book();
        let mut factor = AskQuoteSlopeStats::new();
        ...
        let x = index_array::<5>();
        let expected = ols_slope(&x, &ask_prices[..5]);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_ask_quote_slope_stats_empty_book() {
        let book = empty_book();
        let mut factor = AskQuoteSlopeStats::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
