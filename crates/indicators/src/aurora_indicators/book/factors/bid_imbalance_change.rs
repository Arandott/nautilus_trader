...
const NAME: &str = "bid_imbalance_change";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;

/// Formula/logic: value = diff_ratio(bid_sum, prev)
/// Inputs: bid_amt
/// Output: state.value (factor value)
#[derive(Debug)]
pub struct BidImbalanceChange {
    state: BookFactorState,
}

impl BidImbalanceChange {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(prev) = self.state.last_bid_amt0 {
            self.state.value = diff_ratio(bid_sum, prev);
...
impl Display for BidImbalanceChange {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for BidImbalanceChange {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for BidImbalanceChange {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::BidImbalanceChange;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_bid_imbalance_change_value() {
        let book = sample_book();
        let mut factor = BidImbalanceChange::new();
        ...
        let bid_sum = sum_depth(&bid_amts, 5);
        let expected = diff_ratio(bid_sum, bid_sum);
        assert!(factor.has_inputs());
        assert_relative_eq!(factor.value(), expected);
    }

    #[test]
    fn test_bid_imbalance_change_empty_book() {
        let book = empty_book();
        let mut factor = BidImbalanceChange::new();
        factor.handle_book(&book);
        factor.handle_book(&book);
