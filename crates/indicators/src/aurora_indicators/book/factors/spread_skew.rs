...
const NAME: &str = "spread_skew";
const WINDOW: usize = 60;
const WINDOW_SECONDARY: usize = 60;
/// Formula/logic: rolling push: spread; value = spread - median
/// Inputs: spread
...
impl SpreadSkew {
    ...
    pub fn update(&mut self, book: &OrderBook) {
        ...
        if let Some(spread) = spread {
            self.state.rolling_a.push(spread);
            let median = self.state.rolling_a.median();
            self.state.value = spread - median;
...
impl Display for SpreadSkew {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}()", NAME)
    }
}

impl Indicator for SpreadSkew {
    fn name(&self) -> String {
        NAME.to_string()
    }
...
impl ValueIndicator for SpreadSkew {
    fn value(&self) -> f64 {
        self.state.value
    }
}
...
mod tests {
    ...
    use super::SpreadSkew;
    use crate::aurora_indicators::ValueIndicator;
    use crate::indicator::Indicator;
    use approx::assert_relative_eq;

    #[test]
    fn test_spread_skew_value() {
        let book = sample_book();
        let mut factor = SpreadSkew::new();
    ...
    #[test]
    fn test_spread_skew_empty_book() {
        let book = empty_book();
        let mut factor = SpreadSkew::new();
        factor.handle_book(&book);
        assert!(factor.has_inputs());
